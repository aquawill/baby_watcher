...
line_bot_api = LineBotApi(
    'LjeubAhlWkeZlnptI/uvGRTmQNyUc7/eyzg9LzzwJURnwNV0f/bgXI9XTe791q0yWNmtab+LU/rW9kLNY5WBP7SEDBMUwGa2AVst/YqK5ZNBdqSb/8L7iOZQNmQ4w/ubkKrsx8JUBn7VW2vXLcuxXwdB04t89/1O/w1cDnyilFU=')
handler = WebhookHandler('04c0d8a7121748667989acf6b8e84574')
...
image_product_folder = ''
joy = False




def resize(input_image_path, basesize):
    ...
    output_image_path = "{}_{}.{}".format(os.path.splitext(input_image_path)[0], basesize, ext)
    img.save(output_image_path)
    return output_image_path


def upload_images(image_file_name):
    image_file_name_1024 = resize(image_file_name, 1024)
    image_file_name_240 = resize(image_file_name, 240)
    print(image_file_name, image_file_name_1024, image_file_name_240)
    url_ori = imgur_upload.imgur_upload(image_file_name)
    url_1024 = imgur_upload.imgur_upload(image_file_name_1024)
    url_240 = imgur_upload.imgur_upload(image_file_name_240)
    print(url_ori, url_1024, url_240)
    return url_ori, url_1024, url_1024


def send_joy_photo(uid, image_file_name):
    url_ori, url_l, url_s = upload_images(image_file_name)
    image_message = ImageSendMessage(
        original_content_url=url_l,
        preview_image_url=url_s
    )
    # line_bot_api.push_message(uid, TextSendMessage(text="Baby is coming!"))
    line_bot_api.push_message(uid, image_message)
    # line_bot_api.push_message(uid, TextSendMessage(text=url_ori))
...
class Photographer(Service):
    ...
    def _make_filename(self, timestamp, annotated):
        ...
        file_path = os.path.expanduser(path % (self._folder, timestamp, self._format))
        return file_path
    ...
    def process(self, message):
        ...
        if joy:
            for member in members:
                t_upload_ori = threading.Thread(target=send_joy_photo, args=(member, filename,), name="t_upload_ori")
                t_upload_ori.start()
            joy = False
        '''
        ...
        if faces:
            ...
            with stopwatch('Saving annotated %s' % filename):
                ...
                del draw
                image.save(filename)
                t_upload_anno = threading.Thread(target=upload_photo, name="t_upload_anno")
                t_upload_anno.start()
        '''
...
class Animator(Service):
    ...
    def process(self, joy_score):
        ...
        # print(joy_score, face_detected)
        if joy_score > 0:
            self._leds.pattern = Pattern.breathe(2000)
            self._leds.update(Leds.rgb_on(Color.blend(JOY_COLOR, SAD_COLOR, joy_score)))
            face_detected = True
        else:
            if not watcher_mode:
                self._leds.pattern = Pattern.breathe(2000)
                self._leds.update(Leds.rgb_pattern((80, 0, 0)))
                face_detected = False
            else:
                self._leds.pattern = Pattern.breathe(100)  # 10 Hz
                self._leds.update(Leds.rgb_pattern(Color.RED))
                face_detected = False
...
def joy_detector(num_frames, preview_alpha, image_format, image_folder,
    ...
    with contextlib.ExitStack() as stack:
        ...
        camera = stack.enter_context(PiCamera(sensor_mode=4, resolution=(1920, 1080)))
        ...
        t_photo = threading.Thread(target=take_photo, name="t_photo")
        t_photo.start()
        '''

        def watcher_mode_Switch():
            global watcher_mode
            while True:
                board.button.wait_for_press()
                board.button.wait_for_release(timeout=2)
        ...
        t_switch = threading.Thread(target=watcher_mode_Switch, name="t_switch")
        t_switch.start()
        ...
        def watch_face():
            while True:
                ...
                if face_detected:
                    ...
                    if watcher_mode:
                        player.play(ALM_SOUND)
                time.sleep(0.5)

        t_face = threading.Thread(target=watch_face, name="t_face")
        t_face.start()
...
def main():
    ...
    parser.add_argument('--image_folder', default='~/Pictures/baby_watcher',
    ...
    try:
        if not os.path.exists(os.path.expanduser(args.image_folder)):
            os.mkdir(os.path.expanduser(args.image_folder))
        image_product_folder = os.path.expanduser(args.image_folder)

        def check_image_counts():
            limit = 1000
            while True:
                image_file_list = []
                for dir_path, dir_names, file_names in os.walk(image_product_folder):
                    for file_name in file_names:
                        image_file_list.append(os.path.join(dir_path, file_name))
                    image_file_list.sort(reverse=True)
                if len(image_file_list) > limit:
                    for f in image_file_list[limit - 1:]:
                        os.remove(f)
                time.sleep(10)

        t_check_image_count = threading.Thread(target=check_image_counts, name="t_check_image_count")
        t_check_image_count.start()
...
def callback():
    with open("message_log.txt", "a", encoding="utf-8") as message_log:
...
def handle_message(event):
    ...
    print(json_body)
    uid = json_body["source"]["userId"]
    ori_image_url, large_image_url, small_image_url = upload_images(last_photo_file_name)
    image_message = ImageSendMessage(
        original_content_url=large_image_url,
...
if __name__ == '__main__':
    ...
    print(members)
    t_flask = threading.Thread(target=app.run, name="t_flask")
    t_flask.start()
    sys.exit(main())
